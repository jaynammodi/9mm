...
class NMensMorris(Game):
    ...
    def is_adjacent(self, position1, position2):
        ...
        return position2 in adjacency_map[position1]

    def get_adjacent_positions(self, position):
        """Return a list of adjacent positions for a given position."""
        adjacency_map = {
            1: [2, 4, 7],
            2: [1, 3, 5],
            3: [2, 6, 9],
    ...
    def is_legal_move(self, board, start, end, player):
        """Check if a move from start to end positions by player is legal."""
        if start not in board or end in board:
            return False  # Invalid start or end position
        if player != board[start]:
    ...
    def get_all_moves(self, board, player):
        """Return all possible moves for a player."""
        moves = []
        for position, value in board.items():
            if value == player:
                ...
                for adj_position in adjacent_positions:
                    if adj_position not in board:
                        moves.append((position, adj_position))
        return moves
    ...
    def randomMove(self, self2):
        ...
        while(upperCap > 0):
            ...
            b = random.randrange(len(self2.cells[a]))
            upperCap -= 1
            if(self2.cells[a][b].button["text"] == ""):
                return a, b
    ...
    #     """
    #     player = self.to_move
    #     _, move = self.max_value(self, player)
    #     return move
    ...
    #         utility, _ = self.min_value(successor, player)
    ...
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.max_value(successor, player)
    ...
    #     """
    #     player = self.to_move
    #     _, move = self.alphabeta_max_value(self, player, float('-inf'), float('inf'))
    #     return move

    # def alphabeta_max_value(self, state, player, alpha, beta):
    #     if state.is_terminal():
    #         return state.utility(player), None
    #     max_utility = float('-inf')
    #     best_move = None
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.alphabeta_min_value(successor, player, alpha, beta)
    #         if utility > max_utility:
    #             max_utility = utility
    #             best_move = move
    ...
    # def alphabeta_min_value(self, state, player, alpha, beta):
    #     if state.is_terminal():
    #         return state.utility(player), None
    #     min_utility = float('inf')
    #     best_move = None
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.alphabeta_max_value(successor, player, alpha, beta)
    #         if utility < min_utility:
    #             min_utility = utility
    #             best_move = move
    ...
    # def alphabeta_cutoff_move(self, depth):
    #     """
    #     AI strategy: Alpha-Beta pruning with depth cutoff
    #     Use the Alpha-Beta pruning algorithm with a depth cutoff to find the best move.
    #     """
    #     player = self.to_move
    #     _, move = self.alphabeta_cutoff_max_value(self, player, float('-inf'), float('inf'), depth)
    #     return move

    # def alphabeta_cutoff_max_value(self, state, player, alpha, beta, depth):
    ...
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.alphabeta_cutoff_min_value(successor, player, alpha, beta, depth - 1)
    #         if utility > max_utility:
    #             max_utility = utility
    #             best_move = move
    #         alpha = max(alpha, utility)
    ...
    # def alphabeta_cutoff_min_value(self, state, player, alpha, beta, depth):
    #     if state.is_terminal() or depth == 0:
    #         return state.utility(player), None
    #     min_utility = float('inf')
    #     best_move = None
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.alphabeta_cutoff_max_value(successor, player, alpha, beta, depth - 1)
    #         if utility < min_utility:
    #             min_utility = utility
    #             best_move = move
    #         beta = min(beta, utility)
    ...
    # def expectimax_cutoff_move(self, depth):
    #     """
    #     AI strategy: Expectimax with depth cutoff
    #     Use the Expectimax algorithm with a depth cutoff to find the best move.
    #     """
    #     player = self.to_move
    #     _, move = self.expectimax_cutoff_max_value(self, player, depth)
    #     return move

    # def expectimax_cutoff_max_value(self, state, player, depth):
    #     if state.is_terminal() or depth == 0:
    #         return state.utility(player), None
    #     max_utility = float('-inf')
    #     best_move = None
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.expectimax_cutoff_average_value(successor, player, depth - 1)
    #         if utility > max_utility:
    #             max_utility = utility
    #             best_move = move
    #     return max_utility, best_move

    # def expectimax_cutoff_average_value(self, state, player, depth):
    #     if state.is_terminal() or depth == 0:
    #         return state.utility(player), None
    #     total_utility = 0
    #     num_moves = 0
    #     for move in state.get_legal_moves():
    #         successor = state.result(move)
    #         utility, _ = self.expectimax_cutoff_max_value(successor, player, depth - 1)
    #         total_utility += utility
    #         num_moves += 1
    #     average_utility = total_utility / num_moves
    #     return average_utility, None
    
    # def ai_move(self):
    #     """
    #     Perform an AI move based on the selected strategy.
    #     """
    ...
    #         move = self.alphabeta_move()
    #     elif self.strategy == 'AlphaBetaCutoff':
    #         move = self.alphabeta_cutoff_move(self.depth_cutoff)
    #     elif self.strategy == 'ExpectimaxCutoff':
    #         move = self.expectimax_cutoff_move(self.depth_cutoff)
    #     else:
    #         move = None
